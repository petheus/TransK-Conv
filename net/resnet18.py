...
from net.transkconv import Conv_block
...
def ResNet(
    ...
    x = Conv_block(64, [7,7], img_input.shape[1], strides=2, use_bias=use_bias, name="conv1_conv")(img_input)
...
def residual_block_v1(
    x, filters, imsize, kernel_size=[3,3], stride=1, conv_shortcut=True, name=None
):
    ...
    if conv_shortcut:
        shortcut = Conv_block(
            filters, [1,1], x.shape[1], strides=stride, name=name + "_0_conv"
        )(x)
        ...
        shortcut = x
    x = Conv_block(filters, kernel_size, x.shape[1], strides=stride, name=name + "_1_conv")(x)
    x = layers.BatchNormalization(
        axis=bn_axis, epsilon=1.001e-5, name=name + "_1_bn"
    )(x)
    x = layers.Activation("relu", name=name + "_1_relu")(x)

    x = Conv_block(filters, kernel_size, x.shape[1], name=name + "_3_conv")(x)
    x = layers.BatchNormalization(
...
def stack_residual_blocks_v1(x, filters, imsize, blocks, stride1=2, name=None):
...
def ResNet18(
    ...
    def stack_fn(x):
        x = stack_residual_blocks_v1(x, 64, 256, 2, stride1=1, name="conv2")
        x = stack_residual_blocks_v1(x, 128, 128, 2, name="conv3")
        x = stack_residual_blocks_v1(x, 256, 64, 2, name="conv4")
        return stack_residual_blocks_v1(x, 512, 32, 2, name="conv5")
